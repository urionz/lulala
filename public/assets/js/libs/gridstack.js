 ...
 * @preserve
*/
//每各移动的距离在gridstack.css中设置data-gs-x-1----为对应的百分比
//每个格子要等比缩放就要在width:4, height:4 ，设置对应的比例
//.grid-stack-item-content  2.5px;正好总计5px间距 class="col-md-12" style="background-color: black; padding: 2.5px"
(function(factory) {
    ...
    GridStackEngine.prototype.xjkGetNode = function(x, y, width, height) {
        var nn = {x: x || 0, y: y || 0, width: width || 1, height: height || 1};
        var collisionNode = _.find(this.nodes, _.bind(function(n) {
            return Utils.isIntercepted(n, nn);
        }, this));
    ...
    GridStackEngine.prototype.xjkCanMoveNode = function(node, x, y, width, height) {
         return true;
        var hasLocked = Boolean(_.find(this.nodes, function(n) { return n.locked; }));
    ...
    GridStack.prototype._updateStyles = function(maxHeight) {
        ...
        if (!this.opts.verticalMargin || this.opts.cellHeightUnit === this.opts.verticalMarginUnit) {
            getHeight = function(nbRows, nbMargins) {
                var result = (self.opts.cellHeight * nbRows + self.opts.verticalMargin * nbMargins) +
                    self.opts.cellHeightUnit;
                    //console.log("_updateStyles getHeight ",result,self.opts.cellHeight ,nbRows ,self.opts.verticalMargin,nbMargins)
                return result;
            };
        } else {
            getHeight = function(nbRows, nbMargins) {
    ...
    GridStack.prototype._prepareElementsByNode = function(el, node) {
        ...
        var  xjkDragOrResize = function(event,ui){
                //var x = Math.floor((ui.position.left / cellWidth/4) / cellWidth);                
                //var y = Math.floor((ui.position.top + cellHeight /4) / cellHeight);
                var beforeX = parseInt(node._beforeDragX/4) * 124;
                var beforeY = parseInt(node._beforeDragY/4) * 124;
                var rowwArrow = 0;
                var colArrow = 0;
                // console.log(" ui.position.left beforeX",ui.position.left,beforeX)
                if(ui.position.left>beforeX){                   
                    rowwArrow = 1
                }else{
                    //console.log("左<-------------")
                    rowwArrow = 0;
                ...
                var x = 0;
                if(rowwArrow){
                    x = Math.ceil((ui.position.left - 20)/ 124);
                }else{
                    x = Math.ceil((ui.position.left + 20)/ 124) - 1;
                }
                
                var y = 0 ;
                if(colArrow){
                    y = Math.ceil((ui.position.top - 20)/ 124);                   
                }else{
                    y = Math.ceil((ui.position.top + 20)/ 124) - 1; 
                }
                ...
                //console.log(ui.position.top,"yyyyyyyyyy",y);
               ...
               if (event.type == 'drag') {
                ...
                if( ui.position.left<0 || ui.position.left>380){
                    xjkObj["node"] = null;                   
                    ...
                    xjkObj["y"] = y;
                    xjkObj["width"] = width;
                    xjkObj["height"] = height;
                }               

                //console.log("xjkObj",y);
        ...
        var onEndMoving = function(event, ui) {           
            if(xjkObj.node){
                //TOD move xjkObj移动碰撞目标的node           
                var targetNode = self.grid.xjkGetNode(xjkObj.x, xjkObj.y, xjkObj.node.width, xjkObj.node.height);    
                var isNextBottomNode =  self.grid.xjkGetNode(xjkObj.x, xjkObj.y - 4, xjkObj.node.width, xjkObj.node.height); 
                if(isNextBottomNode){
                    var tmpx = isNextBottomNode.x ,tmpy = isNextBottomNode.y ,tmp_width = isNextBottomNode.width, tmp_height = isNextBottomNode.height;
                }
                     
                if(targetNode){
                    //目标位置和拖动的交互位置
                    self.grid.moveNode(targetNode,node.x,node.y,targetNode.width,targetNode.height,false);
                }               

                //console.log("onEndMoving targetNode",targetNode)
                //console.log("onEndMoving xjkObj",xjkObj)                        
                
                //拖动的移动到目标位置        
                self.grid.moveNode(node, xjkObj.x, xjkObj.y,node.width,node.height,false);    

                //为了防止下面为空，会自动挤到上面去的问题,会把要挤到上的去的网格移动到它原来的位置，目前只做等大小的处理
                if(targetNode&&isNextBottomNode &&(isNextBottomNode._id != node._id) &&(isNextBottomNode.width == node.width)&&(isNextBottomNode.height == node.height)){
                    //isNextBottomNode.locked = true;
                    //self.grid.locked(isNextBottomNode.el,true);     
                    //isNextBottomNode._grid.locked(isNextBottomNode.el,true);  
                    self.grid.moveNode(isNextBottomNode,tmpx,tmpy,tmp_width,tmp_height,false);         
                }    
            }
...
app.directive('gridstack', ['$timeout', function($timeout) {
  ...
  return {
    ...
    link: function(scope, element, attrs, controller, ngModel) {
      ...
      element.on('click', function(e, items) {
        $timeout(function() {
          scope.$apply();
          scope.onClick({event: e, items: items});
        });
      });
